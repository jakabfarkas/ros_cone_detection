...
def build_map(position, detections): # slam elso bemenet
    ...
    min_angle -= a
    max_angle -= a
    current_map, _, _ = draw_line(current_map, x, y, min_angle, max_dist/map_w*map_w_px, (169,169,169), True)
    current_map, _, _ = draw_line(current_map, x, y, max_angle, max_dist/map_w*map_w_px, (169,169,169), True)
    ...
    for i in range(len(cones)-1):
        for j in range(i+1, len(cones)):
            if (merged_cone == False and math.sqrt((cones[i][1]-cones[j][1])**2 + (cones[i][2]-cones[j][2])**2) < min_cone_distance_px):
        ...
        if (isinstance(cones[0], list)):
            for cone in cones:
                if (cone[3] >= 10):
                    ...
                    cv2.circle(current_map, (cone[1],cone[2]), cone_size_px, color, cone_size_px*2)
        else:
            if (cones[3] >= 10):
