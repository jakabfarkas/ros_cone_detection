...
def thresholds(classId):
    if (classId == 0):
        lower_1 = np.array([110, 180, 80], np.uint8)
        upper_1 = np.array([130, 255, 255], np.uint8)
        lower_2 = np.array([0, 140, 0], np.uint8)
        upper_2 = np.array([180, 255, 60], np.uint8)
    if (classId == 1):
        lower_1 = np.array([20, 200, 40], np.uint8)
        upper_1 = np.array([40, 255, 255], np.uint8)
        lower_2 = np.array([0, 200, 40], np.uint8)
        upper_2 = np.array([20, 255, 255], np.uint8)
...
def add_dots(cut_cone, x1, y1, line_contours, dot_list, frame):
    max_y = 0
    ...
    for i in range(len(dot_list)-new_dot_cnt, len(dot_list)):
        dot_list[i][2] = height
        dot_size = int(round(height/20))
        cv2.circle(frame, (dot_list[i][0],dot_list[i][1]), dot_size, (0,255,0), dot_size*2)
    
    return dot_list, height, frame
...
def callback(img):
        ...
        if (len(contour_areas_1) >= 2 and len(contour_areas_2) >= 3):
            ...
            if (len(line_contours) >= 4):
                ...
                if (len(filtered_line_contours) == 4):
                    dot_list, height, frame_vis = add_dots(cut_cone, x1, y1, filtered_line_contours, dot_list, frame_vis)
                    x = x1+xc
                    y = y1+yc
                    detection_list.append([classId, x, y, height])
                    
                    if (classId == 0):
                        rect_color = (255, 0, 0)
                    if (classId == 1):
                        rect_color = (0, 216, 255)
                    xr1 = int(round((x)-height*0.8))
                    yr1 = int(round((y)-height*1.6))
                    xr2 = int(round((x)+height*0.8))
                    yr2 = int(round((y)+height*1.5))
                    rect_thickness = int(round(height/20))
                    cv2.rectangle(frame_vis, (xr1, yr1), (xr2, yr2), color=rect_color, thickness=rect_thickness)
            #else:
    ...
    scale_percent = 50
    width = int(frame_vis.shape[1] * scale_percent / 100)
    height = int(frame_vis.shape[0] * scale_percent / 100)
    dim = (width, height)
    frame_vis = cv2.resize(frame_vis, dim, interpolation = cv2.INTER_AREA)
    cv2.imshow("camera_detected", frame_vis)
    cv2.waitKey(1)
